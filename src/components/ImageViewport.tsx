...
export const ImageViewport: React.FC<ImageViewportProps> = ({
  ...
  /**
   * A synthetic cursor only makes sense with a real mouse.
   * On touch there's no hover to replace, and forcing
   * cursor:none would just delay the tap feedback. Checked
   * once on mount rather than per-move — a device doesn't
   * change pointer type mid-session.
   */
  ...
  const useCustomCursor = hasFinePointer && !shouldReduceMotion;

  const [cursorPosition, setCursorPosition] = useState<CursorPosition | null>(
    null,
  );

  const handleStageMouseEnter = (event: React.MouseEvent<HTMLDivElement>) => {
    if (!useCustomCursor) {
      return;
    }
  ...
  return (
    <>
      <PhotoFrame
        ...
        /*
         * Hides the native cursor only once the custom
         * one has an actual position to render at — never
         * bare `useCustomCursor`, or scrolling the report
         * under an already-stationary mouse (no mousemove
         * fires) would hide the cursor with nothing there
         * to replace it.
         */
        stageClassName={useCustomCursor && cursorPosition ? "cursor-none" : ""}
              ...
              >
                ...
                <AnimatePresence>
                  {showInfo && (
                    ...
                    >
                      A simplified visual approximation. SNIFF does not
                      reproduce a dog&apos;s full sensory experience.
                    </motion.div>
                  )}
                </AnimatePresence>
              </div>
      ...
      {/*
       * Rendered as PhotoFrame's sibling, not its child:
       * the photograph's stage scales via CSS transform for
       * Dog View, and a `position: fixed` descendant of a
       * transformed ancestor is pinned to that ancestor's
       * box instead of the viewport. Staying outside it is
       * what lets clientX/clientY map directly to screen
       * position with no extra math.
       */}
      <AnimatePresence>
        {useCustomCursor && cursorPosition && (
          <motion.div
            aria-hidden="true"
            ...
            className="pointer-events-none fixed z-[60] flex h-16 w-16 -translate-x-1/2 -translate-y-1/2 flex-col items-center justify-center gap-1 rounded-full border border-white/25 bg-[#1D1C19]/85 text-center font-data text-[7px] font-medium uppercase leading-none tracking-[0.1em] text-white shadow-[0_6px_20px_rgba(0,0,0,0.35)] backdrop-blur-xs"
          >
            <span className="h-1 w-1 rounded-full bg-[#6C785B]" />
            <span>{isDogView ? "Dog View" : "Original"}</span>
          </motion.div>
        )}
      </AnimatePresence>
    </>
